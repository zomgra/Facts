        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var roleToUser = model.RoleName; // change to 'IdentityConfiguration.Customer' for cancel choosing role
                    if (!await _roleManager.RoleExistsAsync(roleToUser)) 
                    {
                        var userRole = new ApplicationRole
                        {
                            Name = roleToUser,
                            NormalizedName = roleToUser,
                        };
                        await _roleManager.CreateAsync(userRole);
                    }

                    await _userManager.AddToRoleAsync(user, roleToUser);
